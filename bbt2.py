...
parser.add_argument(
    "--parallel", action="store_true", help="Whether to allow parallel evaluation"
)
parser.add_argument(
    "--inference_framework",
    default="pt",
...
parser.add_argument(
    "--onnx_model_path", default=None, type=str, help="Path to your onnx model."
)
...
if model_name in ["t5-small", "t5-base", "t5-large", "t5-3b"]:
    ...
    from metrics.metrics_t5 import (
        SST2Metric,
        AGNewsMetric,
        YelpPMetric,
        DBPediaMetric,
    ...
    from metrics.metrics_gpt import (
        SST2Metric,
        AGNewsMetric,
        YelpPMetric,
        DBPediaMetric,
    ...
    from metrics.metrics_cpt import (
        ChnSentMetric,
        AmazonMetric,
        THUCNewsMetric,
        BQMetric,
...
if inference_framework == "ort":
    ...
    ), "Path to onnx model is required, got None instead."
    assert os.path.exists(
        onnx_model_path
    ), f"In valid onnx model path `{onnx_model_path}`"
...
class LMForwardAPI:
    def __init__(
        ...
        if cat_or_add == "cat":
            ...
            print(
                "Shape of initial prompt embedding: {}".format(self.init_prompt.shape)
            )
        else:
        ...
        if random_proj == "normal":
            ...
            if model_name in ["roberta-base", "roberta-large"]:
                embedding = (
                    self.model.roberta.get_input_embeddings().weight.clone().cpu()
                )
                ...
                embedding = (
                    self.model.electra.get_input_embeddings().weight.clone().cpu()
                )
            elif model_name in [
                "facebook/bart-base",
                "facebook/bart-large",
                ...
                embedding = (
                    self.model.transformer.get_input_embeddings().weight.clone().cpu()
                )
            else:  # T5
            ...
            print(
                "[Embedding] mu: {} | std: {} [RandProj]  mu: {} | std: {}".format(
        ...
        if task_name == "sst2":
            ...
            self.metric = CCPMMetric(
                target="labels", pred="logits", tokenizer=tokenizer
            )
            self.metric_key = "acc"
            self.metric_name = "CCPMMetric"
        elif task_name == "tnews":
            self.metric = TNewsMetric(
                target="labels", pred="logits", tokenizer=tokenizer
            )
            self.metric_key = "acc"
    ...
    def calc_metric(self, logits, target):
        ...
        if self.metric_key == "acc":
            ...
            perf = f1_score(
                converted_target.detach().cpu().numpy().tolist(),
        ...
        if self.loss_type == "hinge":
            loss = hinge_loss(
                logits, converted_target, margin=self.margin, reduction="sum"
            ).item() / len(target)
        elif self.loss_type == "ce":
            loss = self.ce_loss(logits, converted_target.long()).item()
    ...
    def _init_sparse_projection(self, sparsity):
        """Initializes the linear layer's weight matrix as a sparse matrix."""
        dense_weights = self.linear.weight.data

        # Create a mask on the same device as the dense weights
        mask = torch.rand(dense_weights.shape, device=device) > sparsity

        # Apply the mask to set most weights to zero
        sparse_weights = dense_weights * mask.float()

        # Put the sparse weights back into the linear layer
        self.linear.weight.data.copy_(sparse_weights)

        # Normalize the non-zero elements
        non_zero_count = torch.count_nonzero(self.linear.weight.data)
        if non_zero_count > 0:
            # Create the scale factor tensor on the same device
            scale_factor = torch.sqrt(torch.tensor(1.0 / (1.0 - sparsity), device=device))
            self.linear.weight.data /= scale_factor

        print(f"[Sparse Projection] Non-zero elements: {non_zero_count} out of {dense_weights.numel()} total.")
    ...
    def eval(self, prompt_embedding=None, test_data=None):
        ...
        if isinstance(prompt_embedding, list):  # multiple queries
            ...
            for pe in prompt_embedding:
                z = torch.tensor(pe).type(torch.float32).to(device)  # z
                z = self.linear(z)  # Az
            ...
            prompt_embedding = prompt_embedding.reshape(n_prompt_tokens, -1).repeat(
                bsz, 1, 1
            )
        ...
        if isinstance(test_data, DataSet):
            ...
            if self.num_call % self.eval_every == 0:
                ...
                if (
                    parallel
                ):  # if we have multiple queries, use the one that achieves minimal loss
                    self.model.set_prompt_embedding(prompt_embedding)
...
def get_data(task_name, tokenizer):
    ...
    if args.cat_or_add == "cat":
        data_bundle = DataLoader[task_name](
            tokenizer=tokenizer, n_prompt_tokens=0
        ).my_load(splits)
    else:
        data_bundle = DataLoader[task_name](
            tokenizer=tokenizer, n_prompt_tokens=n_prompt_tokens
        ).my_load(splits)
    return data_bundle
...
def construct_true_few_shot_data(train_data, k_shot):
    ...
    if model_name in ["t5-small", "t5-base", "t5-large", "t5-3b"]:
        new_train_data.set_input(
            "input_ids", "attention_mask", "decoder_input_ids", "decoder_attention_mask"
        )
        new_dev_data.set_input(
            "input_ids", "attention_mask", "decoder_input_ids", "decoder_attention_mask"
        )
    elif model_name in ["gpt2", "gpt2-medium", "gpt2-large", "gpt2-xl"]:
...
if task_name in ["agnews", "yelpp", "dbpedia", "snli"]:
    train_data, test_data = data_bundle.get_dataset("train"), data_bundle.get_dataset(
        "test"
    )
else:
    train_data, test_data = data_bundle.get_dataset("train"), data_bundle.get_dataset(
...
if model_name in ["t5-small", "t5-base", "t5-large", "t5-3b"]:
    train_data = {
        "input_ids": torch.tensor(
            train_data["input_ids"].get(list(range(len(train_data))))
        ),
        "attention_mask": torch.tensor(
            train_data["attention_mask"].get(list(range(len(train_data))))
        ),
        "decoder_input_ids": torch.tensor(
            train_data["decoder_input_ids"].get(list(range(len(train_data))))
        ),
        "decoder_attention_mask": torch.tensor(
            train_data["decoder_attention_mask"].get(list(range(len(train_data))))
        ),
        "labels": torch.tensor(train_data["labels"].get(list(range(len(train_data))))),
    }
    dev_data = {
        "input_ids": torch.tensor(
            dev_data["input_ids"].get(list(range(len(dev_data))))
        ),
        "attention_mask": torch.tensor(
            dev_data["attention_mask"].get(list(range(len(dev_data))))
        ),
        "decoder_input_ids": torch.tensor(
            dev_data["decoder_input_ids"].get(list(range(len(dev_data))))
        ),
        "decoder_attention_mask": torch.tensor(
            dev_data["decoder_attention_mask"].get(list(range(len(dev_data))))
        ),
    ...
    train_data = {
        ...
        "mask_pos": torch.tensor(
            train_data["mask_pos"].get(list(range(len(train_data))))
        ),
        "labels": torch.tensor(train_data["labels"].get(list(range(len(train_data))))),
    }
    dev_data = {
